...
# Add parent dir to sys.path to import config and database
...
class MainApplication(ctk.CTk):
    def __init__(self, mt5_credentials):
        super().__init__()
        self.mt5_credentials = mt5_credentials
        
        self.title("Dicky2K Grid 1.0")
        self.geometry("1280x720")
        self.configure(fg_color=BG_COLOR)
        
        # --- Top Bar ---
        self.top_bar = ctk.CTkFrame(self, height=40, fg_color="black", corner_radius=0)
        self.top_bar.pack(side="top", fill="x")
        self.top_bar.pack_propagate(False)

        # Title in Top Bar
        ctk.CTkLabel(self.top_bar, text="DICKY 2K GRID 1.0", 
                     text_color=ACCENT_YELLOW, font=ctk.CTkFont(size=12, weight="bold")).pack(side="left", padx=10)

        # Status in Top Bar
        status_frame = ctk.CTkFrame(self.top_bar, fg_color="transparent")
        status_frame.pack(side="right", padx=10)
        ctk.CTkLabel(status_frame, text="ACCT: " + self.mt5_credentials.get("account", "N/A"), font=ctk.CTkFont(size=10)).pack(side="left", padx=5)
        ctk.CTkLabel(status_frame, text="SRV: ONLINE", text_color=ACCENT_GREEN, font=ctk.CTkFont(size=10)).pack(side="left", padx=5)
        
        # --- Tab Buttons ---
        self.tab_frame = ctk.CTkFrame(self, height=40, fg_color="transparent")
        self.tab_frame.pack(side="top", fill="x", padx=10, pady=5)
        
        self.btn_dashboard = ctk.CTkButton(self.tab_frame, text="[F1] DASHBOARD", fg_color=ACCENT_YELLOW, text_color="black", hover_color="#cca000", width=120, command=self.show_dashboard)
        self.btn_dashboard.pack(side="left", padx=5)
        
        self.btn_settings = ctk.CTkButton(self.tab_frame, text="[F2] SYSTEM SETTINGS", fg_color=PANEL_BG, text_color="white", hover_color="#333333", width=150, command=self.show_settings)
        self.btn_settings.pack(side="left", padx=5)

        # Logout Button
        self.btn_logout = ctk.CTkButton(self.tab_frame, text="[LOG OUT]", fg_color="transparent", text_color=ACCENT_RED, hover_color="#330000", width=80, command=self.logout)
        self.btn_logout.pack(side="right", padx=5)

        # Emergency Button
        self.btn_emergency = ctk.CTkButton(self.tab_frame, text="[ESC] EMERGENCY", fg_color="transparent", text_color=ACCENT_RED, hover_color="#330000", width=120)
        self.btn_emergency.pack(side="right", padx=5)
        ...
        # Create Views
        self.dashboard_view = DashboardView(self.content_frame, self)
    ...
    def show_dashboard(self):
        ...
        self.btn_dashboard.configure(fg_color=ACCENT_YELLOW, text_color="black")
        self.btn_settings.configure(fg_color=PANEL_BG, text_color="white")

    def show_settings(self):
        self.dashboard_view.pack_forget()
        self.settings_view.pack(fill="both", expand=True)
        self.btn_settings.configure(fg_color=ACCENT_YELLOW, text_color="black")
        self.btn_dashboard.configure(fg_color=PANEL_BG, text_color="white")
...
class DashboardView(ctk.CTkFrame):
    def __init__(self, master, app_main):
        ...
        self.lbl_balance = self.create_stat_box(self.stats_frame, "BALANCE", "$0.00")
        self.lbl_balance.master.pack(side="left", fill="both", expand=True, padx=2)
        
        self.lbl_equity = self.create_stat_box(self.stats_frame, "EQUITY", "$0.00")
        self.lbl_equity.master.pack(side="left", fill="both", expand=True, padx=2)
        
        self.lbl_floating = self.create_stat_box(self.stats_frame, "FLOATING P&L", "+$0.00", color=ACCENT_GREEN)
        self.lbl_floating.master.pack(side="left", fill="both", expand=True, padx=2)
        
        self.lbl_drawdown = self.create_stat_box(self.stats_frame, "DRAWDOWN", "0.00%", color=ACCENT_YELLOW)
        self.lbl_drawdown.master.pack(side="left", fill="both", expand=True, padx=2)
        
        self.lbl_winrate = self.create_stat_box(self.stats_frame, "WIN RATE", "0.0%", color=ACCENT_GREEN)
        self.lbl_winrate.master.pack(side="left", fill="both", expand=True, padx=2)
        
        self.lbl_open_pos = self.create_stat_box(self.stats_frame, "OPEN POS", "0")
        self.lbl_open_pos.master.pack(side="left", fill="both", expand=True, padx=2)
        ...
        self.status_bar_lbl = ctk.CTkLabel(self.status_bar, text="● STATUS: OFFLINE", text_color="gray", font=ctk.CTkFont(size=10, weight="bold"))
    ...
    def create_stat_box(self, parent, title, value, color="white"):
        ...
        lbl_val = ctk.CTkLabel(frame, text=value, font=ctk.CTkFont(size=20, weight="bold"), text_color=color)
        lbl_val.pack(pady=(0, 10))
        return lbl_val

    def update_stats_loop(self):
        """Update Dashboard Stats every 15 seconds"""
        try:
            if not mt5.initialize():
                mt5.initialize() # Try again
            ...
            if acc:
                ...
                equity = acc.equity
                pnl = equity - balance
                dd = abs(pnl / balance * 100) if balance > 0 else 0
                
                self.lbl_balance.configure(text=f"${balance:,.2f}")
                self.lbl_equity.configure(text=f"${equity:,.2f}")
                
                pnl_text = f"{'+' if pnl >= 0 else ''}${pnl:,.2f}"
                self.lbl_floating.configure(text=pnl_text, text_color=ACCENT_GREEN if pnl >= 0 else ACCENT_RED)
                self.lbl_drawdown.configure(text=f"{dd:.2f}%")
                
                # Win Rate from Database
                stats = database.get_performance_summary(hours=24)
                if stats:
                    self.lbl_winrate.configure(text=f"{stats['win_rate']:.1f}%")
                
                # Open Positions
                pos = mt5.positions_get()
                open_count = len(pos) if pos else 0
                self.lbl_open_pos.configure(text=str(open_count))
                
                # Update Status Bar in Main App
                if hasattr(self.app_main, 'status_bar_lbl'):
                    self.app_main.status_bar_lbl.configure(text="● STATUS: ONLINE", text_color=ACCENT_GREEN)
            else:
                if hasattr(self.app_main, 'status_bar_lbl'):
                    self.app_main.status_bar_lbl.configure(text="● STATUS: MT5_DISCONNECTED", text_color=ACCENT_RED)
        except Exception as e:
            print(f"Error updating stats: {e}")
            
        # Schedule next update in 15 seconds
        self.after(15000, self.update_stats_loop)
...
if __name__ == "__main__":
    app = MainApplication({"account": "12345678", "server": "Test"})
    app.mainloop()
