...
try:
    if os.path.exists('settings.json'):
        with open('settings.json', 'r', encoding='utf-8') as f:
            ...
            # 🛡️ Map Grid settings into Profiles
            grid_layers = _user_settings.get("GRID_MAX_LAYERS")
            grid_step = _user_settings.get("GRID_STEP_PIPS")
            for sym_profile in _PROFILES.values():
                if grid_layers is not None: sym_profile["max_layers"] = grid_layers
                if grid_step is not None: sym_profile["layer_step_pips"] = grid_step
