...
class SelfLearningEA:
    ...
    def notify(self, msg, telegram=True):
        """ ส่งข้อความแจ้งเตือนผ่าน Console และ Telegram """
        print(f"\n📢 {msg.replace('*','')}")
    ...
    def check_trade_safety(self, symbol, direction, price, is_scalp=False, smc_zone=None, ignore_spacing=False):
        ...
        s_cfg = config._PROFILES.get(symbol, config._PROFILES["XAUUSDc"])
        min_d = s_cfg.get('layer_step_pips', 15) * pip
        magics = [config.MAGIC_NUMBER]

        all_now = mt5.positions_get(symbol=symbol) or []
        
        # Grid/Layering Limit
        grid_enabled = getattr(config, 'ENABLE_GRID', True)
        max_t = s_cfg.get('max_layers', 5) if grid_enabled else 1
        
        if len(all_now) >= max_t:
            self.log_throttled(f"🚫 [{symbol}] Limit Reached: {len(all_now)}/{max_t} positions. (Grid: {'Enabled' if grid_enabled else 'Disabled'})", symbol)
            return False
    ...
    def _send_order(self, symbol, direction, lot, sl, tp, magic, comment, pending_type=None, pending_price=0.0, expiry_hours=4):
        ...
        for attempt in range(max_retries):
            ...
            if res and res.retcode == mt5.TRADE_RETCODE_DONE:
                ...
                return res.order
            if res and res.retcode in [mt5.TRADE_RETCODE_REQUOTE, mt5.TRADE_RETCODE_CONNECTION, mt5.TRADE_RETCODE_PRICE_OFF, mt5.TRADE_RETCODE_PRICE_CHANGED]:
    ...
    def execute_trade(self, symbol, direction, atr_val, order_index=1, pending_type=None, pending_price=0.0, smc_zone=None, comment=None, box_high=0.0, box_low=0.0, manual_tp=0.0, ai_conf=0):
        ...
        if use_multi_tp:
            ...
            if ticket:
                self.last_execution_time[(symbol, direction)] = time.time()
                s_cfg = config._PROFILES.get(symbol, config._PROFILES["XAUUSDc"])
                max_t = s_cfg.get('max_layers', 5) if getattr(config, 'ENABLE_GRID', True) else 1
                self.notify(f"🟢 *[{symbol}] {direction}* #{order_index}/{max_t}\n"
